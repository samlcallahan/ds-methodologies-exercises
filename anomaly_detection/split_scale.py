...
def scale(df, scaler='standard'):
    if scaler == 'uniform':
        scaler = QuantileTransformer(output_distribution='uniform', random_state=123, copy=True).fit(df)
    elif scaler == 'robust':
        ...
        scaler = StandardScaler(copy=True, with_mean=True, with_std=True).fit(df)
    else:
        print("WARNING: INVALID SCALER")
        return None, df

    train_scaled = pd.DataFrame(scaler.transform(df)).set_index([df.index.values])
    df = train_scaled
